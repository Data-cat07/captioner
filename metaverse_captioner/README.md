...
앱이 학생 PC에서 재생되는 시스템 소리를 WASAPI loopback으로 캡처하고, OpenAI Realtime transcription으로 한국어 자막을 만든 뒤 필요하면 선택 언어로 번역해서 화면 아래 작은 창에 표시합니다. 메타버스 플랫폼을 수정하지 않고 별도 자막창으로 쓰는 구조입니다.
...
2. 이 앱을 실행합니다.
3. 언어를 고릅니다.
4. `Start`를 누르면 화면 아래 자막창이 동작합니다.
